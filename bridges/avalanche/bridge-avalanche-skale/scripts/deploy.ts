...
import { AvalancheSkaleBridge } from "../typechain-types";
...
async function main() {
  ...
  // Deploy the bridge
  const Bridge = await ethers.getContractFactory("AvalancheSkaleBridge");
  ...
  console.log("\nSupported Tokens:");
  for (const token of tokens) {
    const info = await bridge.getTokenInfo(token.address);
    console.log(`\nToken: ${token.address}`);
